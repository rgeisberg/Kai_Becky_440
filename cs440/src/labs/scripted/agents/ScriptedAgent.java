...
public class ScriptedAgent
    ...
    public ScriptedAgent(int playerNum, String[] args) {
        ...
        // even though we have fields for my unit's Id, the enemy unit's Id, and the
        // gold resource (not a unit)'s id
        // we don't have access to that information in the constructor. So, we are
        // forced to rely on the
        // initialStep method (where we have access to the state of the game) to set
        // these fields.
    ...
    public Map<Integer, Action> middleStep(StateView state,
        ...
        UnitView unitView = state.getUnit(MyUnitId);

        int xpos = unitView.getXPosition();
        int ypos = unitView.getYPosition();
        boolean enemyAlive = state.getUnit(this.getEnemyUnitId()) != null;
        boolean goldAlive = state.getResourceNode(this.getGoldResourceNodeId()) != null;
        ...
        if (goldAlive) {
            ...
            if (xpos < 7) {
                ...
                actions.put(MyUnitId, Action.createPrimitiveMove(MyUnitId, Direction.NORTH));
            } else if (xpos == 7 && ypos == 1 && enemyAlive) {
                actions.put(MyUnitId, Action.createPrimitiveAttack(MyUnitId, this.getEnemyUnitId()));
            }
        }
